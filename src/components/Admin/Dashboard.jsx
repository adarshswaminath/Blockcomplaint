...
/**
 *
 * @admin to change the ownership of the contract
 */
function Ownership() {
  ...
  return (
    <div>
      <div className="text-center justify-center">
        {/* form */}
        <div className="bg-gray-200 border-gray-600 border-b-4 border-r-4  p-8 border  m-auto w-96 rounded-lg">
          ...
          <div className="mt-2">
            ...
            <Web3Button
                ...
                action={(contract) => {
                  contract.call("TransferOwner", newOwnerAddress);
                }}
              >
                Transfer OwnerShip
              </Web3Button>
...
function Dashboard() {
  ...
  const handleCount = async (contract) =>
    setCount(await contract.call("TotalCompalints"));
  // handle responsedashboard
  // total complaints update status
  const handleTotalStatus = async (contract) =>
    setStatusUpdate(await contract.call("TotalStatusUpdate"));
  const handleLastResponse = async (contract) =>
    setLastResponseId(await contract.call("LastRespondedComplaint"));
  useEffect(() => {
  ...
  function Respond() {
    ...
    return (
      <div className="p-3 ">
        <button
          className="font-bold py-2 px-4 rounded mb-1"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-200">
          <div className="shadow-lg p-6 rounded-lg text-center justify-center items-center border border-gray-600">
            <div className="mb-4">
              <input
                type="text"
                className="p-3 rounded-lg text-center border border-gray-600"
                placeholder="ID"
            ...
            <div className="mb-4">
              <textarea
                ...
                cols="30"
                rows="10"
                onChange={handleSubmitResponse}
                className="text-center rounded-lg border border-gray-600"
          ...
          {data && (
            <div className="p-6 rounded-lg shadow-md border-2 border-gray-600">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="text-center">
                  ...
                  <p
                    className={`text-2xl font-bold ${
                      data[10] ? "text-green-500" : "text-red-600"
                    }`}
                  >
                ...
                <div>
                  <p className="mb-2 font-semibold text-gray-500 text-sm uppercase">
                    Complaint
                  </p>
                ...
                <div>
                  ...
                  <p className="text-lg">
                    {ethers.BigNumber.from(data[4]._hex).toString()}
                  </p>
                </div>
                <div>
                  <p className="mb-2 font-semibold text-gray-500 text-sm uppercase">
  ...
  return (
    <div className="min-h-screen">
      <div className="heading p-4">
        ...
        <h1 className="text-2xl lg:text-6xl font-semibold">Welcome Back 👋</h1>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 text-white p-4">
        {/* div flex the card */}
        {/* card 1 */}
        ...
        <div className="card flex-1 text-center p-8 rounded-lg bg-gray-700">
          ...
          <button onClick={() => handleLastResponse(contract)}>
            {" "}
            Last Responded Complaint ID
          </button>
        </div>
        {/* card 3 */}
        <div className="card flex-1 text-center p-8 rounded-lg bg-gray-700">
          <span className="font-bold text-4xl"></span>
          <button className="font-bold" onClick={() => setShowResponse(true)}>
            Respond
          </button>
        </div>
        {/* card 4  */}
        <div className="card flex-1 text-center p-8 rounded-lg bg-gray-700">
          <span className="font-bold text-4xl">
            {ethers.BigNumber.from(totalStatusUpdate).toString()}{" "}
          </span>
          <button onClick={() => handleTotalStatus(contract)}>
            Status Updated Complaints
          </button>
        </div>
        {/* card 5  */}
        <div className="card flex-1 text-center p-8 rounded-lg bg-gray-700">
          <span className="font-bold text-4xl"></span>
          <button className="font-bold" onClick={(e) => setOwnerShip(!ownership)}>
            Ownership Transfer
          </button>
        </div>
